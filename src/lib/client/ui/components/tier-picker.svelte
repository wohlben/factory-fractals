...
{#if (tierSelector)}
	<div>
		...
		{#each FactoryGlobals.availableTiers[$recipe.Type] as availableTier}
			<button class="px-2 py-1 outline-slate-600 text-sm  rounded" class:outline-1={availableTier === $tier}
	...
	<button
		...
		class="text-center min-w-16 h-10 rounded-l-lg bg-slate-800 py-2 px-3 flex justify-around items-end gap-2 relative ">
